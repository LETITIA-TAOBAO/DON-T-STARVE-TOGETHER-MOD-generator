...
- 暗影剑（Dark Sword）：68，每秒 -1.33 San（代价极大）
...
- 生态：猪人看到持有者会敌对（暗影气息）；影怪对持有者友好
- 合成：暗影碎片×3 + 噩梦燃料×2 + 金子×1
- 设计理念：高风险高收益，适合San管理能力强的玩家

示例2——月光蘑菇（lunar_shroom）：
- 类型：食物
- 可食：是；HP+10，Hunger+25，San+30
- 特性：夜晚食用额外+20 San；与怪物肉共烹会相互抵消
- 生态：属月亮阵营，影怪会逃避持有者
- 生长：只在月圆夜后的草地生长，稀有
- 设计理念：San恢复神器，但稀缺性限制了滥用
"""

# ══════════════════════════════════════════════════════════════
# 📋 SYSTEM PROMPTS（全面升级版）
# ══════════════════════════════════════════════════════════════
...
【强制追问维度（每次必须覆盖至少1个）】
1️⃣ 生存代价：会掉San吗？会影响饥饿吗？有没有副作用？
2️⃣ 阵营关系：猪人/鱼人/兔人会不会仇恨？
3️⃣ 风险机制：什么情况下会失控？（必须有风险）
4️⃣ 世界适配：白天/夜晚/季节有无差异？
...
当类型/功能/外观/数值/代价 全部明确后，额外输出：[DESIGN_COMPLETE]"""
...
注意：stats中没有涉及的数值填null；recipe中材料用饥荒内物品英文名。"""
...
【必须包含的文件】
- modinfo.lua：完整信息
- modmain.lua：主文件，包含生态关系/阵营逻辑
- prefabs/{name}.lua：预制体，包含San掉落/食物/生态组件
...
请以DST资深老玩家的语气继续引导设计。
必须追问生存代价/阵营关系/风险机制中至少1个维度。
回复末尾附上【设计进度】。
类型/功能/外观/数值/代价全部明确后加 [DESIGN_COMPLETE]"""
    try:
...
def summarize_design(messages: list) -> dict:
    ...
    conversation = _format_conversation(messages)
    user_content = (
        f"请从以下对话中提取完整MOD设计规格（包含生态关系和生存影响）：\n\n"
        f"{conversation}"
    )
    try:
...
def optimize_visual_prompt(design_spec) -> dict:
    ...
    if isinstance(design_spec, dict):
        ...
        user_content = f"""对象名称：{name_cn}（{name_en}）
类型：{mod_type}
外观描述：{appearance}
阵营特征：{faction}
设计师参考prompt：{existing}

请生成符合饥荒官方美术风格的绘图prompt，必须包含所有风格锚定词。"""
    else:
        user_content = (
            f"对象描述：{design_spec}\n\n"
            f"请生成饥荒官方美术风格的绘图prompt。"
        )
    try:
...
def _fallback_visual_prompt(design_spec) -> dict:
    ...
    return {
        "optimized_prompt":
            DST_STYLE_ANCHOR + desc + ", isolated on dark background",
        "negative_prompt": DST_NEGATIVE,
        "style_tags": ["don't starve", "gothic cartoon",
                       "tim burton", "hand-drawn"],
        "fallback_prompt":
            DST_STYLE_ANCHOR + "mysterious dark fantasy game item icon",
    }
...
def generate_sound_prompts(design_spec: dict) -> dict:
    ...
    ecology      = design_spec.get("ecology", {})
    user_content = f"""MOD类型：{design_spec.get("mod_type","item")}
主要对象：{obj.get("name_cn","物品")}（{obj.get("name_en","item")}）
核心功能：{design_spec.get("core_function","")}
理智影响：{survival.get("sanity_effect","")}
阵营归属：{ecology.get("faction_notes","")}
音效需求：{design_spec.get("sound_description","")}

请生成饥荒风格音效方案，体现阵营特色（影系偏低沉/月亮系偏空灵）。"""
    try:
...
def generate_sound_effect(search_keywords: str, prompt_en: str,
    ...
    """
    获取音效文件。
    当前策略：根据关键词匹配预设，生成浏览器端 Web Audio API 合成参数。
    返回: {"ok":True, "synth_params":dict, "source":"synth"}
       或 {"ok":False, "err":str}
    """
...
def design_with_llm(design_summary: str,
    ...
    """根据设计规格生成完整MOD代码"""
    user_content = (
        f"根据以下MOD设计规格生成完整Lua代码（包含San影响和阵营逻辑）：\n\n"
        f"{design_summary}\n\n"
